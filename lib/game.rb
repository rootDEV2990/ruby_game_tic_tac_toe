...
require_relative '../lib/player.rb'
  ...
  class Game
    ...
    def who_turn 
        @turn_1 = [@player_1.name, @player_2.name].sample
    ...
    def player_move 
      until draw? || @winner != ""
        if @turn_1 == @player_1.name
          (@count_turn.even?) ? play_game(@player_2) : play_game(@player_1)
        elsif @turn_1 == @player_2.name
          (@count_turn.even?) ? play_game(@player_1) : play_game(@player_2)
        end
      end
    ...
    def valid_move(player) 
      ...
      until valid_input?(input)
        ...
        puts "This number is out of range (1-9)" unless (input > 0 && input < 10)
    ...
    def valid_input?(input) 
      return input > 0 && input < 10 && !@board.position_taken?(input - 1)
    end
  end
